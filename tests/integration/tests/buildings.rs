...
//!
//! Two questions, and they are separate. The first is whether the generator puts
//! buildings where a town has them — beside the streets, clear of the road surface,
//! clear of each other, and the same every time. The second is whether the two
//! formats that can carry a footprint actually carry *this* footprint: these tests
//! read the written file back through the format's own parser and compare what comes
//! out against what went in, which is the only comparison worth making.
...
fn every_preset_builds_a_town_that_survives_every_export() {
    for (name, _) in PRESETS {
        ...
        // The two formats that can hold a footprint.
...
fn an_osm_reader_gets_the_footprints_back() {
    ...
    for way in &ways {
        ...
        assert!(way.tags.contains_key("building:levels"));
        assert!(way.tags.contains_key("height"));
    }

    // Each footprint's corners come back where they were put, to the centimetre an
    // OSM latitude is worth.
    let mut matched = 0;
    for building in map.buildings.iter() {
        let wanted = building.footprint.points();
        let found = ways
            .iter()
            .find(|way| {
    ...
    assert_eq!(matched, map.buildings.len());
}
...
fn an_opendrive_reader_gets_the_footprints_back() {
    ...
    for object in &objects {
        let outline = object.outline.as_ref().expect("a building has an outline");
        ...
        // The outline's own corners enclose the area the footprint did — which is
        // the thing `cornerRoad` would have lost.
        ...
        let corners: Vec<(f64, f64)> = outline
            ...
            .map(|corner| match corner {
                Corner::Local(local) => (
                    local.u.get::<uom::si::length::meter>(),
                    local.v.get::<uom::si::length::meter>(),
                ),
        ...
        assert!(
            (twice_area.abs() / 2.0 - building.footprint.area()).abs() < 0.01,
            "{name}: {} m² written for a {} m² footprint",
            twice_area.abs() / 2.0,
...
fn the_viewer_draws_the_town_the_file_describes() {
    ...
    let areas: Vec<_> = drawing
        ...
        .collect();
    assert_eq!(areas.len(), map.buildings.len());

    // The picture is of the file, so a footprint drawn from it should land where the
    ...
    assert!(drawing
        .notes
        .iter()
        .any(|note| note.contains("buildings, drawn from the outline")));
}
