...
/// Where a map comes from: the builder its roads are added to, or a file it was
/// read from — one or the other, so a map read from a file has nothing to add to.
enum Source {
    Built(Box<MapBuilder>),
    /// The map as read, and what the reading could not keep.
    Read {
...
impl Source {
    fn metadata(&self) -> &MapMetadata {
        match self {
            Source::Built(builder) => builder.metadata(),
            Source::Read { map, .. } => &map.as_map().metadata,
        }
    }
}
...
impl PyMap {
    ...
    fn new(
        ...
        Ok(PyMap {
            source: Source::Built(Box::new(MapBuilder::new(metadata))),
            built: None,
            buildings: None,
        })
    }
    ...
    fn __repr__(&self) -> String {
        format!(
            ...
            self.source.metadata().name,
            self.source.metadata().projection.as_str(),
            self.source.metadata().handedness.as_str()
        )
    }
}
...
impl PyMap {
    ...
    fn builder_mut(&mut self) -> PyResult<&mut MapBuilder> {
        match &mut self.source {
            Source::Built(builder) => Ok(builder),
            Source::Read { .. } => Err(PyValueError::new_err(
    ...
    fn build(&self) -> PyResult<UnvalidatedMap> {
        let mut map = match &self.source {
            Source::Built(builder) => (**builder).clone().finish().map_err(value_error)?,
            Source::Read { map, .. } => (**map).clone(),
        };
